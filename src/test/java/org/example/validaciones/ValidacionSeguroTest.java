...
class ValidacionSeguroTest {
    ...
    public void ValidarNombreIncorrectoTipoCaracteres(){
        ...
        Assertions.assertEquals(Mensajes.SUPERA_NUMERO_CARACTERES.getMensaje(),resultado.getMessage());
    }
    @Test
    ...
    public void ValidarBeneficiarioCorrecto(){
        String beneficiarioPrueba = "Andres";
        Boolean resultado = Assertions.assertDoesNotThrow(()->this.validacionSeguro.validarBeneficiario(beneficiarioPrueba));
        Assertions.assertTrue(resultado);
    }

    @Test
    public void ValidarBeneficiarioLongIncorrecto(){
        String BeneficiarionPrueba = "andres florez zapata es millonario y no quiere a perra flaca";
        Exception resultado = Assertions.assertThrows(Exception.class,()->this.validacionSeguro.validarBeneficiario(BeneficiarionPrueba));
        Assertions.assertEquals(Mensajes.BENEFICIARIO_LONGITUD.getMensaje(),resultado.getMessage());
    }


    @Test
    public void ValidarBeneficiarioFormatoIncorrecto(){
        String BeneficiarionPrueba = "andres124rty567468  45678";
        Exception resultado = Assertions.assertThrows(Exception.class,()->this.validacionSeguro.validarBeneficiario(BeneficiarionPrueba));
